...
fn parseFenBoardRow(fenBoardRow: []const u8, rank: Rank, board: *Board) !void {
    ...
    for (fenBoardRow) |char| {
        switch (char) {
            ...
            '1'...'8' => {
                file = file.rightN(@truncate(char - '0')) catch return FenError.BoardRowLengthExceeded;
            },
            'p', 'n', 'b', 'r', 'q', 'k', 'P', 'N', 'B', 'R', 'Q', 'K' => {
                ...
                const piece = (if (isUpper) Piece.fromUppercaseAscii(char) else Piece.fromLowercaseAscii(char)) orelse unreachable;
